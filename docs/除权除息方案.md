...
- 定时配置（环境变量，`web/excalendar.go`）：`EXCAL_HEAVY_INTERVAL_HOURS`（xdxr全市场，默认24）；巨潮/touzid自动循环已删除，仅手动 `refresh` 可触发；touzid默认屏蔽，需 `EXCAL_TOUZID=1` + `TOUZID_COOKIE` 才启用。
...
### 抓取器与定时（`web/excalendar.go`，首次启动后5分钟跑一次）
...
- 环境变量：`EXCAL_HEAVY_INTERVAL_HOURS`（默认24）。巨潮/touzid自动循环已删除，仅手动 `refresh` 可触发。
